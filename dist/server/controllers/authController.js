...
authController.createUser = async (req, res, next) => {
    ...
    try {
        ...
        // create new user
        const hashedPassword = await bcrypt_1.default.hash(password, SALT_WORK_FACTOR);
...
authController.verifyUser = async (req, res, next) => {
    ...
    try {
        ...
        // validate password
        const isAuthenticated = await bcrypt_1.default.compare(password, user.password);
        if (!isAuthenticated) {
            return next({
...
authController.updateUser = async (req, res, next) => {
    ...
    try {
        ...
        const isAuthenticated = await bcrypt_1.default.compare(oldPass, user.password);
        if (!isAuthenticated) {
            return next({
    ...
    if (newPass) {
        try {
            ...
            const hashedPassword = await bcrypt_1.default.hash(newPass, SALT_WORK_FACTOR);
...
authController.setSessionCookie = (req, res, next) => {
    ...
    const sessionToken = jsonwebtoken_1.default.sign({ id, firstName, lastName, email, authProvider }, JWT_SECRET);
...
authController.verifySessionCookie = (req, res, next) => {
    ...
    jsonwebtoken_1.default.verify(kst, JWT_SECRET, (err, decoded) => {
        if (err) {
            return next({
