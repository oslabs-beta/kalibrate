...
consumerController.getMessages = async (req, res, next) => {
    ...
    try {
        ...
        messageConsumer.on(messageConsumer.events.GROUP_JOIN, async ({ payload }) => {
        ...
        const removeBatchListener = messageConsumer.on(messageConsumer.events.END_BATCH_PROCESS, async ({ payload }) => {
            ...
            if (Object.values(consumedTopicPartitions).every(consumed => Boolean(consumed))) {
                let cachedMessages = server_1.consumerCache.get(id, clientId, topic); // get current value of cache
                if (cachedMessages)
                    cachedMessages = [...cachedMessages].reverse(); // reverse copy to get most recent first
                res.locals.topicMessages = cachedMessages;
                // end consumer instances and clear cache after 20min timeout
                setTimeout(async () => {
                    await messageConsumer.disconnect();
                    server_1.consumerCache.delete(id, clientId, topic);
        ...
        await messageConsumer.run({
            eachMessage: async ({ messageTopic, partition, message }) => {
                const topicMessage = {
                    topic: messageTopic,
                    partition: partition,
                ...
                server_1.consumerCache.add(id, clientId, topic, topicMessage); // add to cache
            },
        });
    }
    catch (err) {
...
consumerController.checkConsumerCache = (req, res, next) => {
    ...
    const cachedMessages = server_1.consumerCache.get(id, clientId, topic); // store if cache hit
    if (cachedMessages)
        res.locals.topicMessages = [...cachedMessages].reverse(); // reverse copy to get most recent first
    return next();
};
