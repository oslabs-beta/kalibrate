...
const ENCRYPT_KEY = process.env.ENCRYPT_KEY || 'key';
    ...
    if (clusters.length) {
        for (const cluster of clusters) {
            ...
            const decryptedPassword = saslPassword
                ? crypto_js_1.default.AES.decrypt(saslPassword, ENCRYPT_KEY).toString(crypto_js_1.default.enc.Utf8)
                : null;
            let kafka;
            if (brokers.length) {
...
clusterController.getClientConnection = async (req, res, next) => {
    ...
    try {
        ...
        if (!clientQuery.length)
            throw new Error('Query for requested client found no records');
...
clusterController.storeClientConnection = async (req, res, next) => {
    ...
    try {
        ...
        if (clientIdMatch.length)
            throw new Error('Client ID already in use for given user');
        if (!sasl) {
            ...
            const encryptedPassword = crypto_js_1.default.AES.encrypt(password, ENCRYPT_KEY).toString();
            await prisma.$transaction(async (prisma) => {
                const cluster = await prisma.cluster.create({
                    data: {
                        clientId,
...
clusterController.deleteClientConnection = async (req, res, next) => {
    ...
    try {
        ...
        await prisma.$transaction(async (prisma) => {
            ...
            if (!clusterId.length)
                throw new Error('Did not find requested cluster');
